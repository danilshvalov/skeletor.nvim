...
local notify = function(message, level_config)
  if type(vim.notify) == "table" then
    ...
    vim.notify(message, level_config.level, { title = "Skeletor" })
  else
    local console_string = string.format("Skeletor: %s", message)
...
log.new = function(config, standalone)
  ...
  obj.use_file = function(file, quiet)
    if file == false then
      if not quiet then
        obj.info("[skeletor] Logging to file disabled")
      end
      ...
      if not quiet then
        obj.info("[skeletor] Logging to file: " .. obj.outfile)
      end
    end
  end
  ...
  local log_at_level = function(level, level_config, message_maker, ...)
    ...
    if config.use_file then
      ...
      if fp then
        ...
        print("[skeletor] Could not open log file: " .. obj.outfile)
      end
    end
